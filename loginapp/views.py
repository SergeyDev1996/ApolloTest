...
from ApolloTest.settings import APOLLO_PASSWORD, APOLLO_EMAIL, APOLLO_LOGIN_URL, APOLLO_API_REQUEST_URL, \
...
class LoginAndSaveCookiesView(APIView):
    ...
    def get_cookies(self, email: str, password: str):
        ...
        try:
            ...
            email_field = driver.find_element(By.XPATH,
                                              '/html/body/div[2]/div/div[2]/div/div[1]/div/div[2]/div/div[2]/div/form/div[2]/div/div/input')
            email_field.clear()
            email_field.send_keys(email)
            # Find and fill in the password field using XPath
            password_field = driver.find_element(By.XPATH,
                                                 '/html/body/div[2]/div/div[2]/div/div[1]/div/div[2]/div/div[2]/div/form/div[3]/div/div[1]/div/input')
            ...
            try:
                after_login_element_xpath = '/html/body/div[2]/div/div[2]/div[2]/div/div[2]/div/div[2]/div[2]/div[1]/div[1]/div[2]/div[3]/div[3]/div/button/div'
                driver.find_element(By.XPATH,
                                   after_login_element_xpath)  # Check for the element that indicates a successful login
                # Retrieve cookies from the Selenium session
                cookies = driver.get_cookies()
                # cookie_string = json.dumps(cookies)  # Convert cookies to a JSON string
                return cookies
    ...
    def get_driver_requests(self, cookies):
        ...
        driver.get(APOLLO_API_REQUEST_URL)# First, get the main page to set the domain
        for cookie in cookies:
            driver.add_cookie(cookie)  # Add each cookie to the driver
        driver.get(APOLLO_API_REQUEST_URL)  # Navigate again to load resources with the cookies
    ...
    # Get cookies after successful login
        cookies = self.get_cookies(email=APOLLO_EMAIL, password=APOLLO_PASSWORD)
        if not cookies:
            return JsonResponse({'error': 'Login failed. Please check your credentials.'},
                                status=status.HTTP_401_UNAUTHORIZED)
        driver_requests = self.get_driver_requests(cookies=cookies)
        is_writen = self.is_request_writen_to_db(cookies=cookies, driver_requests=driver_requests)
        if is_writen:
            return JsonResponse({'message': 'Request successfully written to the database.'},
                                status=status.HTTP_200_OK)
        return JsonResponse({'message': 'The needed request was not found and information was not written'})
